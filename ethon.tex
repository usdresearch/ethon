...
\title{Ethon -- The Pointer Particle}
\author{Shuhei Ihara\\
\small written in the formal posture of a doctoral thesis\\
\small under the trans-temporal supervision of Hideki Yukawa}
\date{\today}
...
\begin{abstract}
This paper specifies the ethon as a real pointer particle of the
theory.  At the first layer of interpretation, the ethon is a
single-particle formulation of interactions among the six quark flavors:
the six flavors are represented as six directional arguments inside one
real particle, and their readable interaction is represented as
three-axis fluctuation.  Each direction also carries a ternary
interaction sign in \(\{-1,0,1\}\), giving \(3^6\) external
flavor-interaction signatures before rotation, polarity, and bonding
data are read.  It is a general meson implementation obtained by
assigning boundary-free operational parameters to Yukawa's meson idea:
not a
replacement for the pion, but a purer meson whose role is to point from
an unread relation to a stable readable entity before any particular
physical boundary, mass, charge state, decay channel, or detector
realization is imposed.  The ethon has three transition axes:
compression--exposition, upward--downward localization, and
stabilization--turnover.  Its fluctuation is simply rotation in this
three-axis frame; no ten-dimensional or superstring-like ambient space is
required by the core implementation.  As a free functor it remains inert
and has no up--down polarity.  When an entity is placed in the
\(T\)-direction, hidden polarity is born: the \(S\)-face turns toward the
\(T\)-direction and the \(N\)-pole points toward \(S\).  This motion is
not directly observable.  When an entity is also placed in the
\(S\)-direction, the ethon enters constant-speed self-rotation.  The
governing law is the Ihara equation, written around the oriented
\(T\to S\) premise--stabilization arrow.  It gives the general meson
update from directional placement to hidden polarity, three-axis
rotation, residue, and readable pointer form.  Ethon bonding, formerly
called \(S\)-molecular bonding, is the state in which two ethons overlap
completely.  At the observer-operation layer, fusion is the only
admissible ethon operation; a fusion whose readability depends on
ignoring an observer-readable discontinuity is reclassified as fission
disregard and stored as residue.  It can be placed by an observer in any
typed space,
including spaces that do not carry an external time flow, but it cannot
be removed; attempted removal only hides the ethon or transfers it to
residue.  Motion in any one directional component is required to perturb
the others.  The only source text cited in this version is Hideki
Yukawa's original meson paper,
\emph{On the Interaction of Elementary Particles. I} \cite{Yukawa1935}.
\end{abstract}

\tableofcontents

% -----------------------------------------------------------------------------
\section{Existence Postulate}
% -----------------------------------------------------------------------------

The ethon is specified here as a real particle of the present theory.
Its existence is a primitive postulate, not a metaphorical convenience and
not a theorem derived from other particles.  The question of how an
ethon is detected by an external physical apparatus is separate from
the question of whether the implementation contains ethons.

\begin{axiom}[Ethon existence]
For every typed space \(X\) and every admissible observer boundary \(b\),
there is a class of ethon instances
\[
  \Eth_b(X).
\]
These instances are real objects of the implementation.  They may be
visible, hidden, or residue-carried at a boundary, but they are not
...
symbolic computation.  It states the additional structure that a real
ethon carries while remaining backward-compatible with those target
theories.

\begin{remark}[Yukawa posture]
This paper treats Hideki Yukawa as the trans-temporal doctoral supervisor
of the project.  This is a formal scholarly posture and a declaration of
lineage: the ethon is developed as a purified general meson after
Yukawa's act of placing a particle-like pointer inside an unread nuclear
relation \cite{Yukawa1935}.  It is not a claim of institutional
supervision, personal endorsement, or historical intention.  The neutral
polymer lemmas below are formal consequences of the ethon notation, not
claims that Yukawa wrote them in this notation.
\end{remark}

\begin{remark}[Name and pronunciation]
The particle is named \emph{Ethon} and pronounced ``Ee-thon.''  The name
is chosen to sound like a simple particle name, to connect the initial
sound to the Ihara equation, and to admit a mythic reading through
Aithon, the burning figure of the Prometheus cycle.  The ethon is not the
tormenting bird of that cycle.  It is the pointer particle that
stabilizes Promethean fire through the \(T\to S\) Ihara equation.
\end{remark}

\begin{definition}[Quark-flavor interaction formulation]
Let the six quark-flavor names be represented by
\[
  \Qflav_6
  =
  \{c,d,b,s,t,u\}.
...
\[
  \phi_Q:\Arg_{\Eth}\twoheadrightarrow\Qflav_6
\]
given by
\[
  \phi_Q(C)=c,\qquad
...
are formulated in one existing ethon as six directional arguments whose
readable interactions are expressed through three-axis rotational
fluctuation.  The ethon is the real pointer particle by which their
six-flavor interaction pattern is formalized.
\end{definition}

\begin{definition}[Ternary interaction signature]
A quark-flavor interaction state has an external ethon signature
\[
...
\[
  \begin{aligned}
  +1&:\text{quark-facing direction},\\
  -1&:\text{antiquark-facing direction},\\
...
flavor-interaction skeleton, not the whole meson identity.
\end{definition}

\begin{definition}[Pointer particle]
A pointer particle is a particle whose primary observable form is not a
mass point or a medium, but an oriented transition:
\[
  \epsilon^{\Obs}_b:
  \text{unread relation}
  \longrightarrow
...
\]
The arrow is part of the particle's observable signature.  An ethon is
the pointer particle specified in this document.
\end{definition}
...
particle of relation, transition, and stabilization.
\end{remark}

\begin{definition}[Pure meson]
A pure meson is a meson stripped of fixed empirical boundary data and
retaining only its operational pointer role:
\[
  \text{unread relation}
  \longrightarrow
...
\]
The ethon is the pure meson of this paper.  It is prior to any
...
\[
  ...
  \{C,D,E,S,T,U\}.
\]
They are also the six roles of the quark-flavor interaction formulation
...
The Ihara equation is the central equation of the implementation.  It is
not merely a notation for transition.  It is the operator that turns
directional argument placement into hidden motion, hidden polarity,
constant-speed rotation, residue, and general meson reading.

\begin{definition}[General meson reading]
A general meson reading at boundary \(b\) is a morphism in the boundary
category \(\GenMes_b\):
\[
  \mu\in
...
interaction obtained at that boundary.  The ethon is the concrete
pointer particle whose update realizes this general meson reading without
first choosing a particular physical mass, charge state, decay channel,
or detector boundary.
\end{definition}
...
state as a general meson pointer:
\[
  \Rel_b(a)
  \xrightarrow{\ \mu_{n+1}\ }
  \Read_b(a).
...
\]
This is the target implementation condition for using real ethons as a
matter theory.  It does not by itself replace the established empirical
content of existing physics.
\end{remark}
...
\begin{example}[Pion compatibility and purification]
...
The pion is a licensed physical descendant of the meson operation.  The
ethon is purer than the pion in exactly one sense: it keeps the
meson-like pointer operation before the operation is fixed to a particular
physical boundary.  Thus
\[
  \pi\text{-meson}
  =
  \text{pure meson}
  +
  \text{licensed physical boundary data},
\]
whereas
\[
  \Eth
  =
  \text{pure meson with three-axis rotational operational parameters}.
\]
...
is separated from absence by the rotational, polarity, and bonding data.
...
tables; it supplies a general meson interaction formulation for reading
many meson species as ternary six-direction signatures refined by one
three-axis ethon frame.
\end{example}
...
The ethon may be used as a pointer-particle hypothesis for quantum
...
The functor is the observed arrow; the ethon is the real pointer
particle whose observation appears as that arrow.
\end{protocol}
...
\begin{enumerate}[leftmargin=*]
  ...
  rather than a pointer particle.
  ...
  \item \textbf{Target overdescent.}  Treating a real ethon of the
  implementation as a licensed object of a specific target theory without
  the required target-domain contract.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*]
  ...
  \(\Sig_{\Eth}\in\{-1,0,1\}^{6}\) for six-quark-flavor interaction
  signs;
  \item single-particle formulation of interactions among the six quark
  flavors through those signatures;
  ...
  both active;
  \item the \(T\to S\)-based Ihara equation as the general meson update
  law;
  ...
  \item physical interpretation only by explicit license.
\end{enumerate}

Thus the ethon is implemented as a real pointer particle: freely
placeable, non-removable, stable across typed spaces, independent of
global time flow, rotationally fluctuating in three axes, and dynamically
coupled across all three rotational axes.

\paragraph{Acknowledgement.}
The author expresses gratitude to Professor Hideki Yukawa for the meson
idea that made this paper's pointer-particle formulation conceivable,
and to OpenAI GPT-5.5 for helping the author connect Yukawa's legacy with
the present ethon formulation.
